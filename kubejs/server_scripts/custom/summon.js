...
function applyStats(entity, stats) {
  try { entity.getAttribute('minecraft:generic.max_health').setBaseValue(stats.health); entity.health = stats.health; } catch (e) { console.log(e); }
  try { entity.getAttribute('minecraft:generic.attack_damage').setBaseValue(stats.attack); } catch (e) { console.log(e); }
}
...
LevelEvents.tick(event => {
  ...
  const _activeOwners = new Map(); // uuid -> { owner, particles: Set }
  ...
  event.level.getEntities()
    ...
    .forEach(summon => {
      ...
      let _particleType = 'minecraft:enchant';
      for (const [, d] of Object.entries(SUMMON_CONFIG.summons)) {
        if (summon.tags.contains(d.tag)) { _particleType = d.particles; break; }
      }
