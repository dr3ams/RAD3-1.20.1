...
const ROOM_EVENTS = [
  ...
  },
  {
    ...
    onStart: (player, _level, _server) => { player.persistentData.roomEventScoreBoost = true; },
  ...
  },
  {
    ...
    onStart: (player, _level, _server) => { player.persistentData.roomEventEliteSurge = true; },
...
CommonAddedEvents.playerChangeDimension(event => {
  ...
  if (fromDim === "lrdynamicdungeon:dungeon_dimension" && player.persistentData.inRaid) {
    ...
    let _chests = player.persistentData.raidChests || 0;
    ...
    let _xpNext = xpToNextRank(newTotalXP);
    ...
    let _killMult = player.persistentData.killMultiplierBonus;
    let killScore = kills * RAID_CONFIG.killScoreWeight;
    let _chestScore = 0; // chests * RAID_CONFIG.chestScoreWeight; [DISABLED FOR TESTING];
  ...
  event.register(
    Commands.literal("raidstats")
      .executes(ctx => {
        ...
        let _bestTimeStr = bestTimeSec > 0 ? `${btMin}:${btSec}` : '§8N/A';
